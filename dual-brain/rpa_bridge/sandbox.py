"""Landlock + Seccomp sandbox for the RPA Bridge process.

Applied BEFORE any Robot Framework import (INV-5). On failure, the
process exits — never runs unsandboxed.

Compared to ``gui_agent/sandbox.py``:
  - Same: ``$HOME`` read-only, scratch dir read-write, XDG_RUNTIME_DIR
  - Added: ``/dev/uinput`` read-write (synthetic keyboard/mouse input)
  - Same seccomp filter (``ioctl`` already in safe list, ``execve`` denied)

The additional ``/dev/uinput`` access is why RPA is always Tier 3.
...
def _apply_landlock(home_dir: str, scratch_dir: str) -> None:
    ...
    # /dev/uinput — synthetic input events for RPA (group-gated)
    if os.path.exists("/dev/uinput"):
        _add_path_rule("/dev/uinput", _LANDLOCK_READ_WRITE)
...
def _apply_seccomp() -> None:
    ...
    try:
        ...
        raise SandboxError(
            "seccomp Python package required for RPA Bridge sandbox. "
...
def apply_rpa_sandbox(home_dir: str, scratch_dir: str) -> None:
    """Apply Landlock + Seccomp sandbox. Must be called BEFORE Robot Framework init.

    Compared to ``apply_gui_sandbox()``:
      - Same: ``$HOME`` (ro), scratch_dir (rw), XDG_RUNTIME_DIR (rw)
      - Added: ``/dev/uinput`` (rw) for synthetic input events
      - Same seccomp filter (``ioctl`` already in safe list)

    Raises ``SandboxError`` on non-Linux or if sandbox setup fails.
    """
    if sys.platform != "linux":
        raise SandboxError(
            "RPA Bridge requires Linux (Landlock kernel 5.13+). "
    ...
    if not _check_landlock_available():
        raise SandboxError(
            ...
            "RPA Bridge cannot run without filesystem sandboxing (INV-5)."
        )
