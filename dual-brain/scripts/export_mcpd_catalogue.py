...
def _emit_supported_actions(entries: list[dict], out_path: Path) -> None:
    ...
    frozenset test_intent_corpus.py imports AND main.py's F-35 rewrite
    guard reads.

    Scope: **every action QB is permitted to emit**. That is the F-35
    floor. mcpd + controller meta + Layer 2 manifest + gui_agent +
    rpa_bridge — everything QB can validly output today. Anything not
    in this set gets rewritten to `system.unsupported` by main.py
    before dispatch (see `main.py::_SUPPORTED_ACTIONS`).

    v6.10 Track O (F-68 finish): GUI + RPA were previously filtered
    out here on the (stale) assumption that QB never emitted them
    directly. Track K un-hid them in the QB catalogue block; this
    completes the fix by also un-hiding them in the F-35 guard.
    Without both halves, Track K's user-facing prompt work was dead
    end-to-end — QB emitted `gui.click`, F-35 rewrote it to
    `system.unsupported` before the dispatcher ever saw it."""
    names = sorted({
