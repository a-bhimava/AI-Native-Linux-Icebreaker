...
def _emit_catalogue_block(entries: list[dict], out_path: Path) -> None:
    ...
    # Only QB-emittable entries: mcpd tools + controller meta. GUI +
    # RPA are dispatched by the controller from higher-level intents;
    # listing them in the QB catalogue would mislead the planner.
    for e in entries:
...
def _emit_supported_actions(entries: list[dict], out_path: Path) -> None:
    ...
    Scope: actions QB is permitted to EMIT — mcpd-provided tools plus the
    controller-side meta escape hatch (system.unsupported). GUI + RPA
    tools are excluded because QB doesn't emit them directly (the
    controller synthesizes those calls from higher-level intents); they
    would fail main.py's `_SUPPORTED_ACTIONS` guard and rewrite to
    system.unsupported. Keeping them out here preserves parity with
    main.py so test_supported_actions_match_controller_source stays green."""
    names = sorted({
