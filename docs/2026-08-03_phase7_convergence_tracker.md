...
| M7.0.3 | iceui audit sink + INV-8 field parity | pending (pulled forward to v6.18 — v6.17 now owns M7.6a-1) | — | v6.18 | — | `test_iceui_audit_written.py` |
| M7.1 | R13 startup enforcement + `docs/spec/trust-tiers.md` | pending | — | v6.18+ | — | `test_r13_startup_refuse.rs` |
| M7.6a (decision) | Pick Option A/B/C/D — chose (A) restore INV-1 in OC via submit_intent per user 2026-08-10 | shipped as v6.17 M7.6a-1 | — | — | — | — |
| M7.6a-1 | OC edition submit_intent — restore INV-1; make v6.16 features reach OC — 9 sub-commits (a-i) | **SHIPPED 2026-08-11** | see below | v6.17 (pulled forward from v6.18) | 2346 controller (68 new) | See F-111 in `incremental/GROUND_TRUTH.md § 7` |
| M7.6a-1a | `submit_intent` MCP tool in mcp_gui_server + minimal AF_UNIX daemon-socket client + INV-1 forwarding lock + 11 tests | shipped 2026-08-10 | `79185ff` | v6.17 | 11 green | `test_mcp_gui_server_submit_intent.py` (all 11) |
| M7.6a-1b | New daemon RPC `intent.run` + `_start_pipeline_worker` refactor (shared with turn.run) + graceful INTERNAL_ERROR fallback when Controller method missing + 11 tests | shipped 2026-08-10 | `b670317` | v6.17 | 11 green | `test_intent_run_daemon_rpc.py` (all 11) |
| M7.6a-1c | `Controller.run_turn_from_intent` (thin wrapper) + `from_intent` kwarg on run_turn_streaming (scoped OC short-circuit bypass + Step-1 skip + synthesized zero-cost qb_response + deep-copy intent + session history record) + 12 tests | shipped 2026-08-10 | `495b280` | v6.17 | 12 green | `test_run_turn_from_intent.py` (all 12) |
| M7.6a-1d | Real GeminiBackend in OC mode via `_try_build_oc_native_qb` + graceful NoOp fallback on missing key or ctor error + optional `OpencodeOcConfig.native_qb` sub-config + 15 tests | shipped 2026-08-10 | `c5a507c` | v6.17 | 15 green | `test_backend_registry_oc.py` (all 15, covers 14 PF-* checks) |
| M7.6a-1e | `gen-oc-config.sh` OC_MODE env var; `submit_intent_only` mode flips every MCP tool to deny + only iceui_submit_intent allow; `legacy_direct` rollback preserves v6.16 shape; 7 tests via subprocess-invoked fake mcpd | shipped 2026-08-10 | `7be0883` | v6.17 | 7 green | `test_gen_oc_config_oc_mode.py` |
| M7.6a-1f | `oc_audit_bridge` native-dispatch ring (deque 500 + 10min TTL + consume-on-match) + McpdClient.call unconditional mark + bridge skip on match + reset-on-init isolation + 15 tests | shipped 2026-08-11 | `173f885` | v6.17 | 15 green | `test_oc_audit_bridge_filter.py` |
| M7.6a-1g | `cx-distro/distro/opencode_prompt_submit_intent.txt` (~100 lines: intent schema + rules + 5 examples) + gen-oc-config.sh injects instructions[] in submit_intent_only + build-iso.sh installs prompt to `/etc/icebreaker/` + 3 tests | shipped 2026-08-11 | `9ebe5fd` | v6.17 | 3 green | `test_gen_oc_config_oc_mode.py::test_*_instructions_*` + `test_opencode_prompt_file_exists_at_source_path` |
| M7.6a-1h | Smoke-gate L3 tool count 24→25 + prompt file presence + OC_MODE lockdown consistency check + first-boot Step 1c `oc_submit_intent` phase (ok/warn/skipped) | shipped 2026-08-11 | `0b8f482` | v6.17 | 0 new | smoke-gate assertions + first-boot phase (both fail-loud on regression, degrade-not-crash) |
| M7.6a-1i | GROUND_TRUTH.md § 1 V6.17 row + § 7 F-111 close-out + § 10 A-1..A-8 dispositions → F-111 + tracker M7.6a-1 rows SHIPPED + audit doc Tier A #3 banner flip | shipped 2026-08-11 | *(this commit)* | v6.17 | 0 new | docs only |
