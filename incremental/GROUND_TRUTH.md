...
| V6.4 | F-33 fsync + R8 harvest gate | **TESTING** — harvest gate PASS first-run on VM (9/9 responses, 0 missing syscalls, 0 SIGSYS); smoke gate ALL GREEN incl. `mcpd links fsync (F-33 write path present)`; QEMU gate PASS L6 (pbd + llama-server + INV-3); ISO hash-verified on Mac. UTM re-test pending: `# create Hello.txt` must succeed (was SIGSYS on v6.3) | `ISO/incremental/v6.4.iso` | `174ef21f…67ed` | — |
