...
| V6.51 | F-36 SSE2 llama-server + R10 ISA rule | **BUILDING** — llama-server rebuilt pure-SSE2 (`-DGGML_AVX=OFF -DGGML_AVX2=OFF -DGGML_FMA=OFF -DGGML_F16C=OFF -DGGML_SSE42=OFF`) so any x86-64 CPU (Sandy Bridge and earlier included) boots pbd cleanly. Smoke gate adds ymm/zmm-must-be-absent assertion. Rule R10 added | — | — | — |
...
- **R10 — Architecture-critical binaries target the LEAST-COMMON-DENOMINATOR ISA (F-36 rule).** Not the build machine's ISA, not the primary test platform's ISA. For x86-64 that means SSE2 (guaranteed) unless we ship per-CPU variants with a runtime dispatcher. For llama-server specifically: `objdump -d` must show zero `ymm`/`zmm` register references — enforced as a smoke-gate assertion. **Any pre-Haswell x86-64 CPU (2013 or earlier) must boot v6.51+ without crash-looping pbd.**
...
| F-36 | 2026-07-08 | v6.4 booted on an Intel i7-3820 (Sandy Bridge-E, 2012, real x86-64 hardware — not Rosetta this time). pbd crash-looped with `status=4/ILL` (SIGILL) 105+ times. Automated GCP gate + Apple Silicon UTM both passed — this CPU class was outside the tested envelope. Downstream symptom in the TUI: "Privileged Brain not available: llama-server health probe failed at http+unix://localhost/health: FileNotFoundError(2, 'No such file or directory')" — because the socket never got bound | V6.4's llama-server was built portable-x86 (F-24 disabled AVX-512) but still required **AVX2**. i7-3820 supports AVX but not AVX2/FMA/F16C/BMI2 — first AVX2 op → SIGILL. F-24's rule "the build machine's native ISA is a liability" was applied to AVX-512 but not one step down. `wait-for-sockets` (line 10 of controller.service, 60 s timeout) waited, gave up, warned, proceeded — proper F-7/R6 fail-visible behavior; controller then latched into `_UnconfiguredBackend` per F-5/F-20 | Rebuild llama-server pure-**SSE2** (`-DGGML_AVX=OFF -DGGML_AVX2=OFF -DGGML_FMA=OFF -DGGML_F16C=OFF -DGGML_SSE42=OFF`). SSE2 is guaranteed on all x86-64 — works from Athlon 64 (2003) forward. Performance hit vs. AVX2 is real (~2×) but acceptable: Rosetta already dominates the latency budget on Apple Silicon UTM (F-25's 600 s pb_timeout), and on native x86 hardware even SSE2 clears 5 min/turn. Smoke gate assertion: `strings mcpd | grep -q fsync` still applies; add a new one for llama-server: `objdump -d llama-server \| grep -qE 'ymm\|zmm' ; test $? -eq 1` (must have zero AVX/AVX2/AVX-512 registers referenced). **Rule (R10): every architecture-critical binary in the ISO must be built against the LEAST-COMMON-DENOMINATOR ISA on the target platform — not the build machine's ISA and not the primary test platform's ISA.** |
