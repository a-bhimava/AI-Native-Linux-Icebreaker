...
| V6.5 | F-35 intent coverage + F-34 ib-debug fix | **TESTING** — harvest gate ALL GREEN (Phase A/B/C, 10/10 responses); corpus tests 9/9 offline pass; smoke gate ALL GREEN; QEMU gate PASS L6 (pbd + llama-server + INV-3); ISO downloading. UTM check pending: `take me to downloads` must show yellow UNSUPPORTED card, not silent fs.list | `ISO/incremental/v6.5.iso` | `040d4952…207f` | — |
