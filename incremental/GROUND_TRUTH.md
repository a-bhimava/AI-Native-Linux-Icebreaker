...
| V6.6 | Multi-arch build system (amd64 + arm64) | **BUILDING** — R11 rule added; config/archs/{amd64,arm64}.conf + Makefile + 5 arch-aware build scripts. First target: arm64 ISO booting in UTM Virtualize mode with <15s NL turn (vs 175s under Rosetta). amd64 regression must not break | amd64: pending · arm64: pending | — · — | — |
