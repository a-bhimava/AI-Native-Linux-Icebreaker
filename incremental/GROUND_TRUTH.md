...
| V6.51 | F-36 CPU-support scope decision + F-24 smoke-gate hardening | **TESTING** — three SSE2/baseline rebuild attempts to support i7-3820 (Sandy Bridge 2012) all crashed with SIGILL in system stdlib code (VM `hypervisor` flag; root cause not fully diagnosed after 3 iterations). Scope decision: ship v6.51 with existing AVX2 build; document Haswell (2013+) as minimum CPU. F-24 assertion in smoke gate hardened to only reject zmm (AVX-512), not ymm (AVX2 which we require). R10 revised. Harvest gate ALL GREEN; corpus 9/9 offline; smoke gate ALL GREEN | `ISO/incremental/v6.51.iso` | `313d03d2…a12bb` | — |
