...
| V6.4 | F-33 fsync + R8 harvest gate | **BUILDING** — seccomp allowlist gets `SYS_fsync` + `SYS_fdatasync`; new `mcpd-harvest.sh` two-phase pre-build gate wired into `build-iso.sh` so a missing syscall aborts the build before any cloud cost; smoke-gate `strings … grep fsync` positive assertion. UTM re-test: `# create Hello.txt` must succeed | — | — | — |
