...
VMs are created as **Spot instances** — same GPUs, ~70% cheaper, almost always available.
If Google reclaims the VM, training auto-resumes from the last checkpoint.

| Item | On-demand | **Spot** | Est. total (Spot) |
|---|---|---|---|
| n1-standard-4 + T4 | ~$0.35/hr | **~$0.11/hr** | ~$0.18 for SFT + DPO |
| g2-standard-4 + L4 | ~$0.70/hr | **~$0.22/hr** | ~$0.29 for SFT + DPO |
| Boot disk 100 GB SSD | ~$0.017/GB/month | same | ~$1.70/month while stopped |
| Egress (download adapter) | ~$0.08/GB | same | < $0.01 |

**Your $300 credit covers thousands of full training runs.**
...
```bash
cd "/Users/aditya/Documents/temp"
...
1. SFT trains for 3 epochs (~1.5 hrs on T4, ~45 min on L4), checkpoint every 200 steps
...
> **Spot preemption:** If Google reclaims the VM mid-training, the screen session dies
> but the latest checkpoint is safe on disk. To resume:
> ```bash
> bash mac_commands.sh create   # new Spot VM, same disk
> bash mac_commands.sh ssh
> bash 02_start_training.sh     # auto-resumes from last checkpoint
> ```
