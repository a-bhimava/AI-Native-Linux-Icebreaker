...
# ── Latest checkpoints ────────────────────────────────────────────────────────
echo "---- Checkpoints ------------------------------------------------"
SFT_CKPTS=$(ls -d training/adapters/sft/checkpoint-* 2>/dev/null | sort -V)
DPO_CKPTS=$(ls -d training/adapters/dpo/checkpoint-* 2>/dev/null | sort -V)
...
if [ -d "training/adapters/sft/final" ]; then
  echo "  SFT FINAL  : SAVED  ✓"
fi
if [ -d "training/adapters/dpo/final" ]; then
